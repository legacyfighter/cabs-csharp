...
public class Tariff
{
  ...
  private readonly Money _baseFee;
  public int BaseFee => _baseFee.IntValue;
  ...
  public static Tariff OfTime(LocalDateTime time)
  {
    ...
    {
      return new Tariff(3.50f, "Sylwester", new Money((InitialBaseFee + 3) * 100));
    }
    else
    {
      ...
      {
        return new Tariff(2.5f, "Weekend+", new Money((InitialBaseFee + 2) * 100));
      }
      else
      {
        ...
        if ((time.DayOfWeek == IsoDayOfWeek.Saturday && time.Hour > 6 && time.Hour < 17) ||
            (time.DayOfWeek == IsoDayOfWeek.Sunday && time.Hour > 6))
        {
          return new Tariff(1.5f, "Weekend", new Money(InitialBaseFee * 100));
        }
        else
        {
          // tydzień roboczy
          return new Tariff(1.0f, "Standard", new Money((InitialBaseFee + 1) * 100));
        }
      }
    }
  }
  ...
  {
    var pricedecimal = new decimal(distance.ToKmInFloat() * KmRate);
    ...
    return new Money(finalPrice) + _baseFee;
  }
}